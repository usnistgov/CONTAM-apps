...
CONTAM.Project.Read = function ()
{
  ...
  while(!current)
  {
    ...
    else if(version < CONTAM.Globals.cversion)
    {
      ...
      {
        rdr.Rewind();
        if(u.C31toC32.upgrade())   // Try to read as ContamW 3.1 file
          break;             // Conversion failed.
      ...
      {
        rdr.Rewind();
        if(u.C32toC33.upgrade())   // Try to read as ContamW 3.2 file
          break;             // Conversion failed.
        prj.ProjectText = u.C32toC33.c33File;
        rdr.Init(prj.ProjectText);
      }
      if(version == 9)
      {
        rdr.Rewind();
        if(u.C33toC34.upgrade())   // Try to read as ContamW 3.3 file
          break;             // Conversion failed.
        prj.ProjectText = u.C33toC34.c34File;
        rdr.Init(prj.ProjectText);
      }
...
CONTAM.Project.run_read = function ()
{
  ...
  prj.rcdat.densACH = rdr.readR4( 1 ); // 3.4
  prj.rcdat.g = rdr.readR4( 0 );
  ...
  prj.rcdat.cfd_cfd_solver = rdr.readR4( 0 );
  prj.rcdat.cfd_smooth = rdr.readR4( 0 );
  prj.rcdat.cfd_conv_uvw = rdr.readR4( 0 );
  prj.rcdat.cfd_conv_t = rdr.readR4( 0 );
