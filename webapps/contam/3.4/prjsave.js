...
CONTAM.Project.run_save = function()
{
  ...
  pst += sprintf("!dens grav%s", prj.EOL ); // CW 3.4
  pst += sprintf("%6.4f %6.4f %s", 
    prj.rcdat.densACH, prj.rcdat.g, prj.EOL);
  ...
  pst += sprintf("!cfd  cfdcnvg   var zref  maxi dtcmo solv smooth   cnvgUVW     cnvgT%s", prj.EOL); // CW 3.0, 3.4
  pst += sprintf("%3d  %9.2e %3d %3d   %5d %3d%s", prj.rcdat.cfd_ctype, 
           prj.rcdat.cfd_convcpl, prj.rcdat.cfd_var, 0, prj.rcdat.cfd_imax, 
           prj.rcdat.cfd_dtcmo, prj.rcdat.cfd_cfd_solver, prj.rcdat.cfd_smooth, 
           prj.rcdat.cfd_conv_uvw, prj.rcdat.cfd_conv_t, prj.EOL);
...
CONTAM.Project.system_save = function()
{
  ...
  for( pa=prj.Ahs0.start; pa; pa=pa.next )
  {
    ...
    if( pa.path_x<1 || pa.path_x>prj.npath  ) 
      error( 2, "Invalid path number, AHS " + pa.nr.toString());
    pst += sprintf("%3d %3d %3d %3d %3d %3d %d %s",
             pa.nr, pa.zone_r, pa.zone_s, pa.path_r,
             pa.path_s, pa.path_x, pa.color, pa.name) + prj.EOL;
...
CONTAM.Project.path_save = function()
{
  ...
  for( i=1; i<=prj.npath; i++ )
  {
    ...
    pst += sprintf("%3d %2d %d %d %d %d %d",
             ppd.icon, ppd.dir, ppd.color, ppd.u_Ht, ppd.u_XY, ppd.u_dP, ppd.u_F );
...
CONTAM.Project.css_save = function()
{
  ...
  for( i=1; i<=prj.ncss; i++ )
  {
    ...
    pst += sprintf("%3d %3d %3d %3d %3d %f %f  %f %f %f  %f %f %f %d %d",
      pss.nr, jz, je, js, jc, pss.mult, pss.CC0, pss.Xmin, pss.Ymin,
      pss.Hmin, pss.Xmax, pss.Ymax, pss.Hmax, pss.color, pss.u_XYZ );
...
CONTAM.Project.pexp_save = function()
{
  ...
  for( i=1; i<=prj.npexp; i++ )
  {
    ...
    for( ncg=0,pocg=px.pogd; pocg; pocg=pocg.next )
      ncg += 1;
    pst += sprintf("%d %d %d %f %d %s%s%s",
      px.nr, px.gen, ncg, px.cgmlt, px.color, prj.EOL, px.desc, prj.EOL);
...
CONTAM.Project.note_save = function()
{
  ...
  for( j=1; j<=prj.nnote; j++ )
  {
    ...
    pst += sprintf("%d %d %s%s", pn.nr, pn.color, pn.note, prj.EOL);
  }
  pst += prj.UNK.toString() + prj.EOL;
  prj.pst += pst;
